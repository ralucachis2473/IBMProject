...
import { TeacherService } from './as-teacher.service';
...
export class AsTeacherComponent implements OnInit {
	...
	public onDeleteClass(classId: number): void {
		this.teacherService.deleteClass(classId).subscribe(
		(response: void) => {
			//this.getClasses();
			this.classes = this.classes.filter(currentclass => currentclass.id !== classId);
